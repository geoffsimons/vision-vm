...
- Simplified `remote_controller.py` to a thin client, with browser automation and continuous monitoring autonomously handled by the VM's Control API.
- Enabled autonomous Region-of-Interest (ROI) detection directly within the VM.
- Added programmatic video seeking to the Control API.
- Refactored global state to separate Region-of-Interest (ROI) data from video telemetry for clearer data models.
- Implemented play/pause functionality in the Control API.

### Fixed
- Resolved 'Fatal server error' in Xvfb by clearing X11 lock files in `reset-chrome.sh` during startup.
- Improved telemetry data robustness against `NaN`/`Inf` values and safeguarded FPS calculation against division-by-zero errors.
